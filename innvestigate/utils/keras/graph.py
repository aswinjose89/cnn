...
def change_activation_fn(layer,
                             keep_bias=True,
                             name_template=None,
                             weights=None,
                             reuse_symbolic_tensors=True,
...
def apply_activation_fn(Xs, activation="softmax"):
    Xs = iutils.to_list(Xs)
    ret = []
    for x in Xs:
        layer, node_index, tensor_index = x._keras_history
        layer_act = change_activation_fn(layer, activation=activation)
        ret.append(layer_act(layer.get_input_at(node_index)))
...
def model_activation_fn(model, activation="linear"):
    return keras.models.Model(inputs=model.inputs,
                              outputs=apply_activation_fn(model.outputs, activation=activation),
                              name=model.name)
